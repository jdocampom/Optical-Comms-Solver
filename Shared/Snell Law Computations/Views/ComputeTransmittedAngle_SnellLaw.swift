//
//  ComputeTransmittedAngle_SnellLaw.swift
//  Optical Comms Solver (iOS)
//
//  Created by Juan Diego Ocampo on 10/25/21.
//

import SwiftUI

struct ComputeTransmittedAngle_SnellLaw: View {
    
    @State var coreRefractiveIndex: String = ""
    @State var claddingRefractiveIndex: String = ""
    @State var incidentAngle: String = ""
    
    var body: some View {
        let data = SnellLawComputation()
        return Form {
            Section(header: Text("Parameters")) {
                HStack{
                    Text("Refractive Index (Core)").customStyle()
                    Spacer()
                    DecimalField(placeholder: "e.g.: 1.6", field: $coreRefractiveIndex)
                }
                HStack{
                    Text("Refractive Index (Cladding)").customStyle()
                    Spacer()
                    DecimalField(placeholder: "e.g.: 1.4", field: $claddingRefractiveIndex)
                }
                HStack {
                    Text("Incident Angle (Deg)").customStyle()
                    Spacer()
                    DecimalField(placeholder: "e.g.: 80", field: $incidentAngle)
                }
            }
            Section(header: Text("Results")) {
                HStack {
                    Text("Incident Angle (Deg)").customStyle()
                    Spacer()
                    Text(data.computeVariable(target: .transmittedAngle, n1: coreRefractiveIndex, n2: claddingRefractiveIndex, theta1: incidentAngle))
                }
            }
        }
        .navigationTitle("Transmitted Angle")
        .navigationBarTitleDisplayMode(.inline)
    }
}

struct ComputeTransmittedAngle_SnellLaw_Previews: PreviewProvider {
    static var previews: some View {
        ComputeTransmittedAngle_SnellLaw()
    }
}
